...
<template>
  <div class="my-music" :class="{'my-music-full': !playerStore.songList}">
    <div class="music-library" v-if="hasMusicAccount || userStore.localOnlyMode">
      ...
      <LibraryList v-if="!userStore.localOnlyMode" v-show="listType1 != 3" class="library-list"></LibraryList>
      <LocalMusicList :folderlist="downloadedMusicFolder" type="downloaded" v-if="!userStore.localOnlyMode && downloadedMusicFolder" v-show="listType1 == 3 && listType2 == 3" class="local-list"></LocalMusicList>
      <LocalMusicList :folderlist="localMusicFolder" :classifylist="localMusicClassify" type="local" v-if="localMusicFolder" v-show="listType1 == 3 && listType2 != 3" class="local-list"></LocalMusicList>
      <div class="no-folder" @click="router.push('/settings')" v-if="!userStore.localOnlyMode && !downloadedFolderSettings && listType1 == 3 && listType2 == 3">去设置下载地址</div>
      <div class="no-folder" @click="router.push('/settings')" v-if="localFolderSettings.length == 0 && listType1 == 3 && listType2 != 3">去设置扫描地址</div>
    </div>
