...
// 顶部音频可视化：从当前播放实例的音频节点上挂一个 AnalyserNode 取频谱，
// 不改动原有播放链路（只是旁路分析），因此对播放本身没有影响。
const BAR_COUNT = 57
const BAR_WIDTH = 3
...
// 无缝衔接模式下播放器自带 Web Audio 图（GainNode → destination），直接旁路取频谱
...
// Howler 使用 Web Audio 解码播放时，可从 bufferSource 上取频谱
...
// 本项目默认走 html5 播放（<audio> 元素），用 captureStream 取音频流再分析
function getMediaStreamAnalyser(playback) {
    ...
    let stream = null
    try {
        stream = capture.call(mediaElement)
    } catch (_) {
        return null
    }
...
function tryAttachAnalyser(retryCount = 0) {
    ...
    try {
        if (playback.__hmWebAudioPlayer) {
