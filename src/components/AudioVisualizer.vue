...
const FALLBACK_LEVEL_SCALE = 0.62
...
function disconnectMediaElementSource() {
    if (mediaElementAnalyser) {
        try { mediaElementAnalyser.disconnect() } catch (_) {}
    }

    if (mediaElementSource) {
        try { mediaElementSource.disconnect() } catch (_) {}
        try {
            mediaElementSource.connect(mediaElementSource.context.destination)
            mediaElementSource.__hmVisualizerOutputMode = 'direct'
        } catch (_) {}
    }

    mediaElementSource = null
    mediaElementAnalyser = null
}
...
function getMediaElementSourceAnalyser(playback) {
    ...
    if (mediaElementSource && mediaElementAnalyser && mediaElementSource.mediaElement === mediaElement) {
        return configureAnalyser(mediaElementAnalyser)
    }

    disconnectMediaElementSource()

    try {
        mediaElement.crossOrigin = mediaElement.crossOrigin || 'anonymous'
    } catch (_) {}

    let source = mediaElement.__hmVisualizerMediaElementSource
    try {
        if (!source || source.context !== context) {
            source = context.createMediaElementSource(mediaElement)
            source.mediaElement = mediaElement
            mediaElement.__hmVisualizerMediaElementSource = source
        }
        ...
        source.__hmVisualizerOutputMode = 'analyser'
        mediaElementSource = source
        mediaElementAnalyser = nextAnalyser
        return nextAnalyser
    } catch (_) {
        try {
            if (source && source.__hmVisualizerOutputMode !== 'direct') {
                source.connect(context.destination)
                source.__hmVisualizerOutputMode = 'direct'
            }
        } catch (_) {}
        mediaElementSource = null
        mediaElementAnalyser = null
...
function getMediaStreamAnalyser(playback) {
    ...
    const mediaAudioContext = getMediaAudioContext()
    if (!mediaAudioContext) return null
...
function tryAttachAnalyser(retryCount = 0) {
    ...
    try {
        if (isHifiPlayback(playback)) {
            ...
            analyser = getHowlBufferAnalyser(playback) || getMediaElementSourceAnalyser(playback) || getMediaStreamAnalyser(playback)
        }
...
function getFallbackSeed() {
    const rawSeed = String(songId.value || '')
    let seed = 0
    for (let i = 0; i < rawSeed.length; i++) {
        seed = (seed * 31 + rawSeed.charCodeAt(i)) % 9973
    }
    return seed / 9973
}
...
function buildFallbackLevels() {
    ...
    const safeProgress = Number(progress.value)
    const timeSeconds = Number.isFinite(safeProgress) && safeProgress > 0
        ? safeProgress
        : performance.now() / 1000
    const safeVolume = Math.max(0.18, Math.min(1, Number(volume.value) || 0.3))
    const seed = getFallbackSeed()
    const beat = Math.pow((Math.sin(timeSeconds * 3.1 + seed * 8) + 1) / 2, 3)
    const pulse = 0.36 + beat * 0.42

    return Array.from({ length: BAR_COUNT }, (_, index) => {
        const position = index / Math.max(1, BAR_COUNT - 1)
        const lowBand = Math.sin(timeSeconds * 2.4 + position * 5.2 + seed * 4)
        const highBand = Math.sin(timeSeconds * 7.4 + position * 18.5 + seed * 13)
        const shape = 1 - Math.abs(position - 0.38) * 1.22
        const energy = 0.18 + Math.max(0, shape) * pulse + Math.max(0, lowBand) * 0.24 + Math.max(0, highBand) * 0.13
        return Math.max(FLAT_LEVEL, Math.min(1, energy * safeVolume * FALLBACK_LEVEL_SCALE))
    })
}
...
function drawFrame() {
    ...
    if (analyserLevels) {
        ...
        const fallbackLevels = buildFallbackLevels()
        if (fallbackLevels) levels.value = fallbackLevels
        else settleFlat()
