<script setup>
  ...
  import { isQQSong, canUseSongAction, getQQSongMid } from '../utils/providerPolicy.mjs'
  ...
  const { currentMusic, playing, progress, playMode, songList, songId, currentIndex, volume, time, playlistWidgetShow, lyricShow, localBase64Img, listInfo, showSongTranslation, widgetState, heartModeArmed } =storeToRefs(playerStore)
  ...
  const canShowSongLike = computed(() => (
    ...
    && (isCurrentQQSong.value ? qqAccountStore.loggedIn : hasUserSession.value)
  ))
  ...
  const checkIsLike = computed(() => (id) => {
    // QQ 用 songmid 查独立的 qqLikelist，网易云沿用 likelist，两者 id 形态不同
  ...
  <div class="music-widget">
    ...
    <div class="music-right">
        ...
        <div class="music-other" :class="{'is-intelligence-mode': isIntelligenceMode}">
            ...
            <svg v-show="isIntelligenceMode || heartModeArmed" @click="changePlayMode()" class="icon intelligence-mode-icon" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-label="心动模式">
